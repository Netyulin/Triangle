...
- VPS 建议把上传目录外置到仓库外，例如：
  - 实际目录：`/opt/triangle-data/uploads`
  - 仓库内软链接：`/opt/triangle/backend/uploads -> /opt/triangle-data/uploads`
- 这样做的好处：
  - `git pull`、`git stash -u`、`git reset --hard` 不会再碰上传文件
  - 文章封面、软件图标、头像、签名素材都能和代码发布解耦
  - 后续备份时只需要单独备份 `/opt/triangle-data/uploads`
...
BAIDU_PUSH_SITE=https://www.sanjiaosoft.com
BAIDU_PUSH_TOKEN=İٶtoken
BAIDU_PUSH_ENDPOINT=http://data.zz.baidu.com/urls
BAIDU_PUSH_TIMEOUT_MS=5000
```
...
```bash
cd /opt/triangle/backend
npm run push:baidu:all
```

ѡ

```bash
npm run push:baidu:all -- --delay-ms=200
```
