...
const TOKEN_HASH_COLUMN = isPostgresDatabase() ? '"tokenHash"' : 'tokenHash';
const EXPIRES_AT_COLUMN = isPostgresDatabase() ? '"expiresAt"' : 'expiresAt';
...
export const verifyResetTokenValidation = validate([
...
function getPublicSiteUrl() {
  const site = normalizeString(process.env.PUBLIC_SITE_URL || process.env.SITE_URL || '').trim();
  return site.replace(/\/$/, '');
}
...
async function findValidResetTokenRecord(rawToken) {
  ...
  const rows = await queryRaw(
    `
    SELECT id, ${USER_ID_COLUMN} AS "userId", ${EXPIRES_AT_COLUMN} AS "expiresAt", ${USED_AT_COLUMN} AS "usedAt"
    FROM password_reset_tokens
    WHERE ${TOKEN_HASH_COLUMN} = ?
    LIMIT 1
    `,
    tokenHash
  );
...
export async function forgotPassword(req, res) {
  ...
  if (user && user.status === 'active') {
    ...
    await executeRaw(
      `UPDATE password_reset_tokens SET ${USED_AT_COLUMN} = CURRENT_TIMESTAMP WHERE ${USER_ID_COLUMN} = ? AND ${USED_AT_COLUMN} IS NULL`,
      user.id
    );

    await executeRaw(
      `INSERT INTO password_reset_tokens (${USER_ID_COLUMN}, ${TOKEN_HASH_COLUMN}, ${EXPIRES_AT_COLUMN}, ${CREATED_AT_COLUMN}) VALUES (?, ?, ?, CURRENT_TIMESTAMP)`,
      user.id,
      tokenHash,
    ...
    try {
      await sendPasswordResetEmail({
        ...
        expiresMinutes: PASSWORD_RESET_TOKEN_EXPIRES_MINUTES
      });
    } catch (error) {
      console.error('[forgot-password] send email failed:', error instanceof Error ? error.message : error);
    }
  }

  return sendSuccess(res, null, '如果该邮箱存在，我们已经发送了重置密码邮件。');
}

export async function verifyResetToken(req, res) {
  await ensurePasswordResetTables();
  const token = normalizeString(req.body.token).trim();
  const record = await findValidResetTokenRecord(token);
  if (!record) {
    return sendError(res, ErrorCodes.TOKEN_INVALID, 'token is invalid or expired');
  }
...
export async function resetPassword(req, res) {
  ...
  return sendSuccess(res, null, 'password reset success');
}
